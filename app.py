...
def main():
    ...
    if 'logged_in' not in st.session_state:
        st.session_state.update({'logged_in': False, 'role': None, 'username': "", 'roll_no': "", 'custom_df': None})
    ...
    if not st.session_state['logged_in']:
        ...
        if app_mode == "Project Overview":
            ...
            if choice == "Login":
                ...
                reg_roll_no = st.text_input("University Roll Number") if role == "Student" else ""
        ...
        try:
            ...
            if user_role in ["Admin", "Mentor"]:
                ...
                fig_risk = px.pie(df, names='Risk_Level', title='Academic Risk Distribution', hole=0.4, color='Risk_Level', color_discrete_map=risk_color_map)
                g1.plotly_chart(fig_risk, use_container_width=True)
                fig_scatter = px.scatter(df, x='Total_%', y='Predicted_GPA', color='Risk_Level', color_discrete_map=risk_color_map, title='Attendance vs Performance Analysis')
                ...
                st.title(f"👋 Welcome to Your Portal, {st.session_state['username']}")
                ...
                if not student_row.empty:
                    ...
                    m1.metric("Attendance Rate", f"{s_row['Total_%']}%"); m2.metric("Projected Sem GPA", s_row['Predicted_GPA']); m3.metric("Academic Status", s_row['Risk_Level'])
                    
                    st.divider()
                    available_features = [col for col in feature_cols if col in df.columns]
                    class_avg, subjects = df[available_features].mean(), [col.split('_')[0] for col in available_features]
                    
                    fig_comp = go.Figure()
                    fig_comp.add_trace(go.Bar(x=subjects, y=s_row[available_features], name='You', marker_color='#004a99'))
                    fig_comp.add_trace(go.Bar(x=subjects, y=class_avg, name='Class Average', marker_color='#d1d1d1'))
                    fig_comp.update_layout(barmode='group', title="Comparative Analysis (You vs Class Average)")
                    st.plotly_chart(fig_comp, use_container_width=True)

                    personal_scores = {col.split('_')[0]: s_row[col] for col in available_features if s_row[col] > 0}
                    if personal_scores:
                        st.divider()
                        fig_personal = px.bar(x=list(personal_scores.keys()), y=list(personal_scores.values()), title="Your Enrolled Subject Proficiency", labels={'x':'Subjects', 'y':'Marks'}, text=list(personal_scores.values()), color_discrete_sequence=['#002b5c'])
